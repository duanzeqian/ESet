...
int main()
{
    ...
    for (int j = 1; j <= M; ++j)
    {
        ...
        for (auto& val : v) s1.upper_bound(val);
        for (auto& val : v) s1.lower_bound(val);
        auto end1 = chrono::high_resolution_clock::now();
        auto start2 = chrono::high_resolution_clock::now();
        for (auto& val : v) s2.upper_bound(val);
        for (auto& val : v) s2.lower_bound(val);
        auto end2 = chrono::high_resolution_clock::now();
        auto start3 = chrono::high_resolution_clock::now();
        for (auto& val : v) s3.upper_bound(val);
        for (auto& val : v) s3.lower_bound(val);
