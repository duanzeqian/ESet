...
private:
    ...
    Node* create_node()
    {
        void* mem = _alloc.allocate(sizeof(Node));
        return new (mem) Node();
    }

    Node* create_node(const Key& key, Color c = RED)
    {
        void* mem = _alloc.allocate(sizeof(Node));
        return new (mem) Node(key, c);
    }
...
public:
    ...
    // Basic functions
    ESet() : NIL(create_node()), rt(NIL), header(create_node()), siz(0), cmp(), _alloc()
    {
        NIL->color = BLACK;
    ...
    ESet(const ESet& other) : NIL(create_node()), rt(NIL), header(create_node()), siz(other.siz), cmp(other.cmp), _alloc()
    {
        *NIL = *(other.NIL);
        NIL->left = NIL->right = NIL->father = NIL;
        header->left = header->right = header;
